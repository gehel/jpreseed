...
public class UsbCreator {
    ...
    public final void create(
            InputStream srcBootImgGz,
            GZIPOutputStream targetBootImg,
            ByteBuffer sysLinuxCfg,
            Collection<Path> preseedFiles) throws IOException {
        try (FatModifier fatModifier = new FatModifier(srcBootImgGz)) {
            ByteBuffer srcInitrdGz = fatModifier.getFileContent(INITRD_GZ);
            srcInitrdGz.rewind();
            fatModifier.addOrReplace(INITRD_GZ, repackedInitrd(srcInitrdGz, preseedFiles));
